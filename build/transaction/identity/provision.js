...
class provisionIdentity extends persistenceJS_1.Persistence {
    constructor() {
        super(...arguments);
        this.provision = async (address, chain_id, mnemonic, identityID, to, feesAmount, feesToken, gas, mode, memo) => {
        ...
        this.createIdentityProvisionMsg = async (address, chain_id, identityID, to, feesAmount, feesToken, gas, memo) => {
            let path = this.path;
            let options = {
                method: "POST",
                url: path + config.provisionKeyPath,
