...
class takeOrder extends persistenceJS_1.Persistence {
    constructor() {
        super(...arguments);
        this.take = async (address, chain_id, mnemonic, fromID, takerOwnableSplit, orderID, feesAmount, feesToken, gas, mode, memo) => {
        ...
        this.createOrderTakeMsg = async (address, chain_id, fromID, takerOwnableSplit, orderID, feesAmount, feesToken, gas, memo) => {
            let path = this.path;
            let options = {
                method: "POST",
                url: path + config.takeOrderPath,
