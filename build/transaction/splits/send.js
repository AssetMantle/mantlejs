...
class sendSplits extends persistenceJS_1.Persistence {
    constructor() {
        super(...arguments);
        this.send = async (address, chain_id, mnemonic, fromID, toID, ownableID, split, feesAmount, feesToken, gas, mode, memo) => {
        ...
        this.createSplitsSendMsg = async (address, chain_id, fromID, toID, ownableID, split, feesAmount, feesToken, gas, memo) => {
            let path = this.path;
            let options = {
                method: "POST",
                url: path + config.sendSplitPath,
